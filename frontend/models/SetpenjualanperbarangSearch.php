<?php

namespace frontend\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use frontend\models\Setpenjualanperbarang;

/**
 * SetpenjualanperbarangSearch represents the model behind the search form of `frontend\models\Setpenjualanperbarang`.
 */
class SetpenjualanperbarangSearch extends Setpenjualanperbarang
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['ralan', 'kelas1', 'kelas2', 'kelas3', 'utama', 'vip', 'vvip', 'beliluar', 'jualbebas', 'karyawan'], 'number'],
            [['kode_brng'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Setpenjualanperbarang::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'ralan' => $this->ralan,
            'kelas1' => $this->kelas1,
            'kelas2' => $this->kelas2,
            'kelas3' => $this->kelas3,
            'utama' => $this->utama,
            'vip' => $this->vip,
            'vvip' => $this->vvip,
            'beliluar' => $this->beliluar,
            'jualbebas' => $this->jualbebas,
            'karyawan' => $this->karyawan,
        ]);

        $query->andFilterWhere(['like', 'kode_brng', $this->kode_brng]);

        return $dataProvider;
    }
}
